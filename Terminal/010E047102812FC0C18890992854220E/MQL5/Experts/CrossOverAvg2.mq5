...
void OnTick()
{
    ...
    /*// Check for Martingale strategy if last trade was a loss
    if (OrdersTotal() > 0)
    {
        for (int i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            if (OrderType() == OP_BUY && orderProfit < 0)
            {
                ...
                {
                    if (trade.Buy(newLotSize, _Symbol, ask, ask - StopLoss * _Point, ask + TakeProfit * _Point))
                    {
                        Print("Martingale Buy: Increased lot size to ", newLotSize);
                        LastLotSize = newLotSize;
                    }
                    else
                    {
                        Print("Error opening martingale buy order: ", GetLastError());
                    }
                }
                else
                {
            ...
            else if (OrderType() == OP_SELL && orderProfit < 0)
            {
                ...
                if (newLotSize <= AccountFreeMarginCheck(_Symbol))
                {
                    if (trade.Sell(newLotSize, _Symbol, bid, bid + StopLoss * _Point, bid - TakeProfit * _Point))
                    {
                        Print("Martingale Sell: Increased lot size to ", newLotSize);
                        LastLotSize = newLotSize;
                    }
                    else
                    {
                        Print("Error opening martingale sell order: ", GetLastError());
                    }
                }
                else
                {
